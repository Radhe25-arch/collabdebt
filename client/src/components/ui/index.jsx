...
export function Button({
  ...
  const base = 'inline-flex items-center justify-center gap-2 font-body font-semibold rounded-lg cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95';
  ...
  const variants = {
    ...
    teal:      'bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm',
...
export function Input({ label, error, icon, className = '', ...props }) {
  return (
    <div className="flex flex-col gap-1 w-full">
      {label && <label className="block text-sm font-semibold text-foreground/80 mb-1.5">{label}</label>}
      <div className="relative">
        ...
        <input
          className={`w-full bg-background border border-border rounded-xl px-4 py-3 placeholder:text-muted-foreground text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all text-sm ${icon ? 'pl-9' : ''} ${error ? 'border-destructive focus:ring-destructive/20' : ''} ${className}`}
          {...props}
        />
      </div>
...
export function Card({ children, className = '', hover = false, ...props }) {
  return (
    <div
      className={`sf-card p-6 ${hover ? 'cursor-pointer hover:border-primary/50 hover:shadow-lg hover:-translate-y-1 transition-all duration-300' : ''} ${className}`}
      {...props}
    >
      {children}
    </div>
...
export function Modal({ open, onClose, title, children, width = 'max-w-lg' }) {
  ...
  return (
    ...
    >
      <div className={`w-full ${width} bg-card border border-border rounded-2xl shadow-2xl animate-fade-up flex flex-col max-h-[90vh]`}>
        {(title || onClose) && (
          <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
            {title && <h3 className="font-display font-bold text-lg text-foreground">{title}</h3>}
            {onClose && (
...
export function StatCard({ label, value, icon, trend, className = '' }) {
  return (
    <div className={`sf-card p-5 ${className}`}>
      ...
      {trend !== undefined && (
        <div className={`text-xs font-mono mt-1 ${trend >= 0 ? 'text-indigo-600' : 'text-destructive'}`}>
